import React from "react";
import AdminLayout from "../../admin/layouts/AdminLayout";
import { usePartnersB2b } from "../hooks/usePartnersB2b";
import { Link } from "react-router-dom";
import { ArrowLeft, ShoppingBag } from "lucide-react";

export default function MarketplacePage() {
  const { partners, loading } = usePartnersB2b();

  if (loading) {
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64 text-slate-500">
          Carregando diretório de marketplace...
        </div>
      </AdminLayout>
    );
  }

  return (
    <AdminLayout>
      <div className="space-y-6 text-left select-none animate-fadeIn text-xs max-w-4xl">
        <Link to="/admin/partners-b2b" className="flex items-center gap-1 text-purple-700 font-bold hover:no-underline text-xs">
          <ArrowLeft size={14} /> Voltar ao Painel
        </Link>

        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 my-0">Marketplace de Parceiros (B2B / B2C)</h1>
          <p className="mt-2 text-sm text-slate-655 my-0">
            Vitrine unificada reunindo todas as categorias de prestadores de serviços turísticos homologados em Curitiba.
          </p>
        </div>

        {/* Partners grid */}
        <section className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4">
          <h3 className="text-lg font-bold text-slate-900 my-0 flex items-center gap-1.5">
            <ShoppingBag size={18} className="text-purple-755 font-bold" /> Empresas Credenciadas
          </h3>

          <div className="divide-y divide-slate-100 font-mono text-[10px]">
            {partners.map(p => (
              <div key={p.id} className="py-4 first:pt-0 last:pb-0 flex justify-between items-center text-sans font-sans">
                <div>
                  <strong className="text-slate-900 text-xs block">{p.name}</strong>
                  <span className="text-[10px] text-slate-505 block">ID: {p.id} | Categoria: {p.category} | Nota: {p.rating} ★</span>
                </div>
                <span className="text-[9px] bg-emerald-50 text-emerald-700 font-bold px-2 py-0.5 rounded border border-emerald-100 uppercase">
                  {p.status}
                </span>
              </div>
            ))}
          </div>
        </section>
      </div>
    </AdminLayout>
  );
}
