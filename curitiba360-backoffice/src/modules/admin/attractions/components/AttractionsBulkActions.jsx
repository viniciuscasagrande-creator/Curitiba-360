...
import { Pencil, RotateCcw, Trash2, X, XCircle } from 'lucide-react';

export function AttractionsBulkActions({
  selectedCount,
  statusTab,
  onEdit,
  onActivate,
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-3 lg:flex-row lg:items-center text-left">
      <strong className="text-sm text-emerald-950 lg:mr-auto">
        {selectedCount} {selectedCount === 1 ? 'atração selecionada' : 'atrações selecionadas'}
      </strong>
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          onClick={onEdit}
          className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40 hover:bg-slate-50"
        >
          <Pencil size={15} />
          Editar
        </button>
...
export default AttractionsBulkActions;
