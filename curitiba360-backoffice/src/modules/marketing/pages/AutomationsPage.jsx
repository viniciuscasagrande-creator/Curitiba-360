...
export default function AutomationsPage() {
  const automations = [
    { id: "aut-001", name: "Carrinho Abandonado", description: "Envia lembrete 30 min depois de abandonar compra.", status: "active" },
    { id: "aut-002", name: "Cupom de Aniversário", description: "Envia cupom automático de 10% no aniversário do cliente.", status: "active" }
  ];
    ...
    <PartnerLayout>
      <div className="mx-auto max-w-5xl space-y-6 select-none text-left">
        <header className="flex items-center justify-between gap-4">
          ...
          <Link
            to="/parceiro/marketing/automacoes/nova"
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          {automations.map((aut) => (
            <div key={aut.id} className="p-4 border border-slate-200 rounded-2xl flex items-center justify-between">
              <div>
                <h4 className="font-bold text-slate-955 my-0">{aut.name}</h4>
                <p className="text-xs text-slate-505 my-0 mt-0.5">{aut.description}</p>
              </div>
              <span className="rounded-full bg-emerald-50 text-emerald-700 font-bold border border-emerald-100 px-2.5 py-0.5 text-xs">
                {aut.status.toUpperCase()}
              </span>
            </div>
          ))}
        </section>
