...
export default function AudienceCreatePage() {
  ...
  const handleSubmit = (e) => {
    ...
    setSuccess(true);
    setTimeout(() => {
      navigate("/parceiro/marketing/publicos");
    }, 2000);
  };
    ...
    return (
      <PartnerLayout>
        <div className="mx-auto max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm select-none">
          ...
          <h2 className="mt-5 text-2xl font-bold text-slate-955 my-0">Público Criado!</h2>
          <p className="mt-2 text-sm text-slate-655 my-0">
            O público foi criado com sucesso e está sendo calculado em segundo plano.
          </p>
        </div>
      </PartnerLayout>
    );
    ...
    <PartnerLayout>
      <div className="mx-auto max-w-xl space-y-6 select-none text-left">
        <header className="flex items-center gap-4">
          <Link
            to="/parceiro/marketing/publicos"
        ...
        <form onSubmit={handleSubmit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
          <div>
            <label className="text-xs font-bold text-slate-700 block mb-1">Nome do Público</label>
            <input
              type="text"
              value={name}
