...
export default function AudiencesPage() {
  const { audiences } = useMarketingDashboard();
    ...
    <PartnerLayout>
      <div className="mx-auto max-w-5xl space-y-6 select-none text-left">
        <header className="flex items-center justify-between gap-4">
          ...
          <Link
            to="/parceiro/marketing/publicos/novo"
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          {audiences.map((aud) => (
            <div key={aud.id} className="p-4 border border-slate-200 rounded-2xl flex items-center justify-between">
              <div>
                <h4 className="font-bold text-slate-955 my-0">{aud.name}</h4>
                <p className="text-xs text-slate-505 my-0 mt-0.5">{aud.description} • Tamanho estimado: {aud.estimatedSize} contatos</p>
              </div>
