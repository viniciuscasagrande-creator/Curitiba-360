import React from 'react';
import { Target, TrendingUp, DollarSign } from 'lucide-react';

export default function CampaignPerformanceGrid({ campanhas = [] }) {
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs space-y-3 text-xs font-medium">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
        <h3 className="font-extrabold text-slate-900 text-xs flex items-center gap-1.5">
          <Target className="w-3.5 h-3.5 text-purple-600" /> Desempenho de Campanhas de Aquisição (ROAS & Vendas)
        </h3>
        <span className="text-[10px] text-slate-400 font-mono">{campanhas.length} ativas</span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {campanhas.map((c) => (
          <div key={c.id} className="p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2">
            <div className="flex items-center justify-between font-extrabold text-slate-900 text-xs">
              <span>{c.nome}</span>
              <span className="px-2 py-0.5 rounded bg-purple-100 text-purple-800 font-bold text-[9px]">
                {c.canal}
              </span>
            </div>
            <div className="text-base font-extrabold text-purple-950 font-mono">
              {c.compras} Vendas atribuídas
            </div>
            <div className="text-[10px] text-slate-500 font-medium">Cliques: {c.Cliques.toLocaleString()} • ROAS: <b className="text-emerald-700">{c.roas}</b></div>
          </div>
        ))}
      </div>
    </div>
  );
}
