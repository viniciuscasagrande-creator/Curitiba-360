...
import RefundsApiPanel from '../components/RefundsApiPanel';
...
export default function RefundsApiPage() {
  ...
  const handleRequestRefund = async (refundData) => {
    const res = await financeApiService.processRefundRequest(refundData);
    alert(res.message);
    loadData();
  };
  ...
  if (loading || !data) {
    return (
      <div className="bg-slate-900 min-h-screen p-12 text-center text-white space-y-3 flex flex-col items-center justify-center">
        ...
        <p className="text-xs text-purple-300 font-semibold animate-pulse">Carregando API de Reembolsos...</p>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto text-xs text-slate-800">
      <div className="flex items-center justify-between border-b pb-4 border-slate-200">
        <h1 className="text-xl font-extrabold text-slate-900 flex items-center gap-2">
          <RefreshCw className="w-6 h-6 text-purple-600" /> API Pública de Reembolsos (`/v1/refunds`)
        </h1>
      </div>

      <RefundsApiPanel refunds={data.refunds || []} onRequestRefund={handleRequestRefund} />
    </div>
  );
}
