...
import { Send, X, AlertCircle, CheckCircle2, DollarSign } from 'lucide-react';
...
export default function PayoutRequestModal({ isOpen, onClose, saldoDisponivel = 0, chavePix = '', onRequestPayout }) {
  ...
  const [solicitante, setSolicitante] = useState('Maria Oliveira');
  ...
  const handleSubmit = async (e) => {
    ...
    if (val > saldoDisponivel) {
      setErrorMsg('Valor solicitado excede o saldo disponível para saque.');
      return;
    }

    setProcessing(true);
    try {
      await onRequestPayout(val, solicitante);
      onClose();
    } catch (err) {
      setErrorMsg(err.message || 'Erro ao solicitar repasse PIX.');
    } finally {
      setProcessing(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-5 shadow-2xl border border-slate-200 space-y-4 animate-fade-in">
        <div className="flex items-center justify-between border-b pb-3 border-slate-100">
          <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
            <Send className="w-4 h-4 text-emerald-600" /> Solicitar Repasse PIX (Diagrama BO-07)
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="p-3 bg-emerald-50/60 rounded-xl border border-emerald-200/60 space-y-1">
            <span className="text-[10px] font-bold text-emerald-800 uppercase">Saldo Disponível para Saque</span>
            <div className="text-xl font-extrabold text-emerald-700">
              R$ {saldoDisponivel.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </div>
            <p className="text-[10px] text-emerald-600">Liquidação imediata na chave PIX cadastrada.</p>
          </div>

          <div>
            <label className="block font-bold text-slate-700 mb-1">Valor da Transferência PIX (R$)</label>
            <input
              type="number"
              step="100"
              max={saldoDisponivel}
          ...
          <div>
            <label className="block font-bold text-slate-700 mb-1">Solicitado por</label>
            <input
              type="text"
              value={solicitante}
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
            ...
            >
              ...
              <span>{processing ? 'Efetuando PIX...' : 'Confirmar Repasse Instantâneo'}</span>
            </button>
          </div>
        </form>
      </div>
